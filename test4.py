...
def extract_and_combine_forces(output_dir, JSON_FOLDER, Tn, Sa, direction, eigs, dmp, scalf, 
                             json_filepath='RSA_Forces.json', csv_filepath='RSA_Forces.csv'):
    """
    ...
    for mode in range(1, len(eigs)+1):
        ...
        for ele_tag in element_tags:
            try:
                # Use extract_beam_results to get forces reliably
                beam_results = extract_beam_results(ele_tag, nep=2, JSON_FOLDER=JSON_FOLDER)
                
                # Get forces at the first evaluation point (typically at one end of the element)
                if beam_results['forces']:
                    forces = beam_results['forces'][0]
                    modal_forces['P'][mode][ele_tag] = forces['N']
                    modal_forces['Vy'][mode][ele_tag] = forces['Vy']
                    modal_forces['Vz'][mode][ele_tag] = forces['Vz']
                    modal_forces['T'][mode][ele_tag] = forces['T']
                    modal_forces['My'][mode][ele_tag] = forces['My']
                    modal_forces['Mz'][mode][ele_tag] = forces['Mz']
                ...
                modal_forces['P'][mode][ele_tag] = 0.0
                modal_forces['Vy'][mode][ele_tag] = 0.0
                modal_forces['Vz'][mode][ele_tag] = 0.0
                modal_forces['T'][mode][ele_tag] = 0.0
                modal_forces['My'][mode][ele_tag] = 0.0
                modal_forces['Mz'][mode][ele_tag] = 0.0
    ...
    with open(csv_filepath, 'w', newline='') as csvfile:
        ...
        # Write header
        writer.writerow(['Element', 'P', 'Vy', 'Vz', 'T', 'My', 'Mz'])
        
        # Write CQC results
        for ele_tag in element_tags:
            writer.writerow([
                ele_tag,
                cqc_forces['P'][ele_tag],
                cqc_forces['Vy'][ele_tag],
                cqc_forces['Vz'][ele_tag],
                cqc_forces['T'][ele_tag],
                cqc_forces['My'][ele_tag],
                cqc_forces['Mz'][ele_tag]
            ])
    # critical_forces = 0
...
def plot(output_dir, JSON_FOLDER, modal_props, Tn, Sa, eigs, nmode, story_heights):
    ...
    # extract_and_combine_forces(output_dir, JSON_FOLDER, Tn, Sa, direction, eigs, dmp, scalf, 
    #                          json_filepath='RSA_Forces.json', csv_filepath='RSA_Forces.csv')
