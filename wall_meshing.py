...
def plot_mesh_data_z_axis(json_folder, location_name, IMAGE_FOLDER):
    """
        ...
        for element_name, element_data, location_name in elements_group:
            ...
            for n in node_names:
                if n in node_dict and n not in plotted_nodes:
                    x, y = node_dict[n][:2]
                    plt.text(x, y, n, fontsize=10, color='green', fontweight='bold')
                    plotted_nodes.add(n)
...
def plot_mesh_data_y_axis(json_folder, location_name, IMAGE_FOLDER):
    """
...
def plot_mesh_data_x_axis(json_folder, location_name, IMAGE_FOLDER):
    """
...
def plot_mesh_data(json_folder, location_name, IMAGE_FOLDER):
    plot_mesh_data_separately(json_folder, location_name, IMAGE_FOLDER)
    plot_mesh_data_z_axis(json_folder, location_name, IMAGE_FOLDER)
    plot_mesh_data_y_axis(json_folder, location_name, IMAGE_FOLDER)
    plot_mesh_data_x_axis(json_folder, location_name, IMAGE_FOLDER)
...
def create_proper_mesh_for_closed_area_3d1(points, predefined_points, shell_section_name, JSON_FOLDER, IMAGE_FOLDER, thickness, load_case_names, pressures, num_x_div, num_y_div, numbering, add_shell, remove_shell, location_name):
    ...
    # plot_mesh_data(JSON_FOLDER, location_name, IMAGE_FOLDER)
...
def example_usage(JSON_FOLDER):
    ...
    # Define expected coordinate levels based on spacing (assuming foot = 1.0 for simplicity)
    foot = 1.0  # Adjust this based on your unit system
    
    # Calculate expected Z levels (floors)
    z_levels = [0.0, 10.0 * foot, (10.0 + 12.0) * foot, (10.0 + 12.0 + 11.0) * foot]
