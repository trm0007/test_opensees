...
# Modified version of the function that checks if ele_tag exists in ele_shapes
def plot_extruded_shapes_3d(ele_shapes, az_el=az_el,
    ...
    # Only process elements that exist in ele_shapes
    for i, ele_tag in enumerate(ele_tags):
        # Skip if the element doesn't exist in ele_shapes
        if ele_tag not in ele_shapes:
            continue
        ...
        try:
            ...
            # Get the shape type and arguments from ele_shapes
            shape_type, shape_args = ele_shapes[ele_tag]
            
            if shape_type == 'rect' or shape_type == 'I' or shape_type == 'L':
                if shape_type == 'rect':
                    ...
                    verts = _plot_extruded_shapes_3d_L(ecrd, g, shape_args)
            ...
            print(f"Warning: Skipping element {ele_tag} due to error: {str(e)}")
            continue
...
def _plot_extruded_shapes_3d_L(ecrd, g, shape_args):
    """
    Generate vertices for an L-shaped extruded member.
    
    Parameters:
    -----------
    ecrd : array
        Element coordinates (2x3 array for start and end nodes)
    g : array
        Local coordinate system transformation matrix (3x3)
    shape_args : list
        [H, B, t] where:
        H = total height of vertical leg (mm)
        B = total width of horizontal leg (mm) 
        t = thickness of both legs (mm)
        
    Returns:
    --------
    verts : list
        List of vertex arrays defining the faces of the L-shaped extrusion
    """
    H, B, t = shape_args
    
    # Half dimensions for easier calculation
    H2, B2, t2 = H/2, B/2, t/2
    
    # Local coordinate offsets for the L-shape vertices
    # Define L-shape in local y-z plane (looking along x-axis)
    # Origin at centroid of L-shape
    
    # Calculate centroid offset from geometric center
    # For L-shape: centroid is at (B*t - t²/2)/(B*t - t²/2) from corner
    A_total = B * t + H * t - t * t  # Total area
    y_centroid = (B * t * t/2 + (H-t) * t * (t + (H-t)/2)) / A_total - H/2
    z_centroid = (t * B * B/2 + t * (H-t) * t/2) / A_total - B/2
    
    # Define 8 vertices of L-shape cross-section relative to centroid
    # Counter-clockwise from bottom-left, outer edge
    vertices_2d = [
        (-H2 - y_centroid, -B2 - z_centroid),  # 0: bottom-left outer
        (H2 - y_centroid, -B2 - z_centroid),   # 1: bottom-right outer  
        (H2 - y_centroid, -t2 - z_centroid),   # 2: bottom-right inner
        (t2 - y_centroid, -t2 - z_centroid),   # 3: corner inner bottom
        (t2 - y_centroid, B2 - z_centroid),    # 4: corner inner top
        (-H2 - y_centroid, B2 - z_centroid),   # 5: top-left outer
        (-H2 - y_centroid, t2 - z_centroid),   # 6: top-left inner
        (-t2 - y_centroid, t2 - z_centroid),   # 7: corner outer
    ]
    
    # Convert to 3D coordinates for both end nodes
    pts = np.zeros((16, 3))
    
    # Start node (i=0) cross-section vertices
    for i, (y_local, z_local) in enumerate(vertices_2d):
        g1y, g1z = g[1, :] * y_local, g[2, :] * z_local
        pts[i] = ecrd[0, :] + g1y + g1z
    
    # End node (i=1) cross-section vertices  
    for i, (y_local, z_local) in enumerate(vertices_2d):
        g1y, g1z = g[1, :] * y_local, g[2, :] * z_local
        pts[i + 8] = ecrd[1, :] + g1y + g1z
    
    # Define faces of the L-shaped extrusion
    verts = [
        # End faces
        [pts[0], pts[1], pts[2], pts[3], pts[4], pts[5], pts[6], pts[7]],  # start face
        [pts[8], pts[9], pts[10], pts[11], pts[12], pts[13], pts[14], pts[15]],  # end face
        
        # Side faces (rectangular faces connecting corresponding edges)
        [pts[0], pts[8], pts[9], pts[1]],    # bottom face
        [pts[1], pts[9], pts[10], pts[2]],   # outer vertical face (bottom part)
        [pts[2], pts[10], pts[11], pts[3]],  # inner horizontal face (bottom)
        [pts[3], pts[11], pts[12], pts[4]],  # inner vertical face
        [pts[4], pts[12], pts[13], pts[5]],  # inner horizontal face (top)
        [pts[5], pts[13], pts[14], pts[6]],  # outer vertical face (top part)
        [pts[6], pts[14], pts[15], pts[7]],  # top face
        [pts[7], pts[15], pts[8], pts[0]],   # outer horizontal face
    ]
