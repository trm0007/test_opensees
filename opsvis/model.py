...
def plot_extruded_shapes_3d(ele_shapes, az_el=az_el,
    ...
    for tag in ops.getEleTags():
        if ops.eleType(tag) == 'ForceBeamColumn3d':
            ele_tags.append(tag)
    # print("ele_tags")
    # print(ele_tags)
    ...
    for i, ele_tag in enumerate(ele_tags):
        ...
        try:
            ...
            if shape_type == 'rect' or shape_type == 'I':
                ...
                if verts:  # Only plot if we got valid vertices
                    ax.add_collection3d(Poly3DCollection(verts, linewidths=0.6,
                                                    edgecolors='k', alpha=.25))
                # ax.plot_surface(X, Y, Z, linewidths=0.4, edgecolors='k',
                #                 alpha=.25)
...
def _plot_extruded_shapes_3d_L(ecrd, g, shape_args):
    """Generate vertices for an L-shaped extruded member using geometric transformation.
    
    Args:
        ecrd: Element coordinates (2 nodes, 3 coordinates each)
        g: Local axis vectors (3x3 matrix)
            g[0,:] - local x-axis (axial direction)
            g[1,:] - local y-axis
            g[2,:] - local z-axis
        shape_args: [H, B, t] where:
            H - total height of L-section
            B - total width of L-section
            t - thickness of legs
    """
    try:
        H = float(shape_args[0])
        B = float(shape_args[1])
        t = float(shape_args[2])
    except (ValueError, TypeError, IndexError) as e:
        print(f"Invalid L-section dimensions: {shape_args}. Error: {str(e)}")
        return []

    # Define vertices in local y-z plane (unrotated)
    vertices_2d = [
        (-H/2, -B/2),    # 0: bottom-left outer
        (H/2, -B/2),     # 1: bottom-right outer
        (H/2, -B/2 + t), # 2: bottom-right inner
        (-H/2 + t, -B/2 + t), # 3: vertical leg inner
        (-H/2 + t, B/2), # 4: top-left inner
        (-H/2, B/2)      # 5: top-left outer
    ]

    # Convert to 3D coordinates for both end nodes
    pts = []
    for i in range(2):  # For both start and end nodes
        for y, z in vertices_2d:
            # The rotation is handled automatically by the g matrix
            # g[1] is local y-axis, g[2] is local z-axis
            pt = ecrd[i] + y * g[1] + z * g[2]
            pts.append(pt)

    # Define faces (same as before)
    verts = [
        pts[:6],   # Start face
        pts[6:12], # End face
        # Side faces
        [pts[0], pts[6], pts[7], pts[1]],
        [pts[1], pts[7], pts[8], pts[2]],
        [pts[2], pts[8], pts[9], pts[3]],
        [pts[3], pts[9], pts[10], pts[4]],
        [pts[4], pts[10], pts[11], pts[5]],
        [pts[5], pts[11], pts[6], pts[0]]
    ]
...
#             t = float(shape_args[2])
#         else:
#             raise ValueError("L-section requires at least 3 parameters (H, B, t)")
            
#         print(f"Processing L-section with H={H}, B={B}, t={t}")
        
#     except (ValueError, TypeError, IndexError) as e:
#         print(f"Invalid L-section dimensions: {shape_args}. Error: {str(e)}")
#         return []  # Return empty list to prevent plotting issues

#     # Define vertices in local y-z plane (origin at geometric center)
#     vertices_2d = [
#         (-H/2, -B/2),    # 0: bottom-left outer
#         (H/2, -B/2),     # 1: bottom-right outer
#         (H/2, -B/2 + t), # 2: bottom-right inner
